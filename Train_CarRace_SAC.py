...
trainlog_path = os.path.join(results_dir, 'trainlog_sac.npy')
training_state_path = os.path.join(results_dir, 'training_state_sac.npy')
replay_buffer_path = os.path.join(results_dir, 'replay_buffer_sac.pkl')

# === Load Checkpoint if Exists ===
checkpoint_files = sorted(glob.glob(os.path.join(model_dir, "model_*_actor")),
                          key=lambda x: int(x.split("_")[-2]))

stepcounter, traincounter, savecounter = 0, 1, 1
trainlog = []
start_episode = 0
...
# === Training Loop ===
for episode in range(start_episode, 40000):
    ob = process_state(env.reset())
    ...
    for step in range(10000):
        ...
        if traincounter % 100000 == 0 and not saved:
            ...
            print(f"SAC model {savecounter} saved at training iteration {traincounter}")
            saved = True
    ...
    if episode_reward_value > best_reward:
        ...
        # policy.save(os.path.join(model_dir, "model_best"))
    ...
    np.save(training_state_path, training_state)
    np.save(trainlog_path, trainlog)
